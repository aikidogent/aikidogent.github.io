...
const buildPug = () => {
  return gulp
    ...
    .pipe(
      data(() => {
        return JSON.parse(
          fs.readFileSync(`${paths.pug.dest}/data.json`).toString()
        );
      })
    )
    .pipe(
...
const watchFiles = () => {
  logWrapper("Pug", () =>
    watch(paths.pug.watchSrc, { usePolling: true }, buildPug)
  );
  logWrapper("Sass", () =>
    watch(paths.styles.src, { usePolling: true }, series(buildSass, lintsass))
  );
  logWrapper("Scripts", () =>
    watch(paths.scripts.src, { usePolling: true }, scripts)
  );
  logWrapper("Assets", () =>
    watch(paths.assets.src, { usePolling: true }, assets)
  );
  logWrapper("PugData", () =>
    watch(paths.pug.data, { usePolling: true }, series(pugData, buildPug))
  );
};
...
// Commands
const buildCommand = series(
  pugData,
  icons,
  parallel(series(buildSass, lintsass), buildPug, scripts, assets)
);
